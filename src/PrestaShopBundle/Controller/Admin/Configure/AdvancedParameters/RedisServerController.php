...
use PrestaShop\PrestaShop\Adapter\Cache\redisServerManager;
 ...
 */
class RedisServerController extends FrameworkBundleAdminController
{
    ...
    {
        return new JsonResponse($this->getredisManager()->getServers());
    }
    ...
    public function addAction(Request $request)
    {
        ...
        if ($postValues->has('server_ip')
            ...
            && $this->getredisManager()->testConfiguration(
                $postValues->get('server_ip'),
                $postValues->getInt('server_port')
            )
        ) {
            $server = $this->getredisManager()
                ->addServer(
                    $postValues->get('server_ip'),
                    $postValues->getInt('server_port'),
                    $postValues->get('server_weight'),
                    $postValues->get('server_base')
                );
        ...
        return new JsonResponse(
            array(
                'errors' => array(
                    $this->trans('The redisd server cannot be added.', 'Admin.Advparameters.Notification'),
                ),
            ),
            Response::HTTP_BAD_REQUEST
        );
    ...
    public function deleteAction(Request $request)
    {
        ...
        if ($request->request->has('server_id')) {
            $this->getredisManager()->deleteServer($request->request->get('server_id'));
        ...
        return new JsonResponse(
            array(
                'errors' => array(
                    $this->trans(
                        'There was an error when attempting to delete the redisd server.',
    ...
    /**
     * @return redisServerManager
     */
    private function getredisManager()
    {
        return $this->get('prestashop.adapter.redis_server.manager');
    }
}
